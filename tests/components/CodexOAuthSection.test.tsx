...
describe("CodexOAuthSection", () => {
  ...
  it("does not render account quota in Auth Center; only renders when showAccountQuota is set", () => {
    // Auth Center 已不再展示账号配额（showAccountQuota 未传，默认 false）
    render(<AuthCenterPanel />);
    expect(screen.queryByTestId("account-quota")).not.toBeInTheDocument();

    // 配额渲染收敛为 CodexOAuthSection 的 prop 能力
