...
let toggleSkillAppVariables:
  | { id: string; app: "grokbuild"; enabled: boolean }
  | undefined;
let bulkToggleSkillAppPending = false;
let bulkToggleSkillAppVariables:
  | { ids: string[]; app: "grokbuild"; enabled: boolean }
  | undefined;
...
describe("UnifiedSkillsPanel", () => {
  ...
  it("enables only disabled Skills from the full list when the app state is mixed", async () => {
    ...
    await user.click(screen.getByText("Grok Build:").closest("button")!);
  ...
  it("disables all Skills when every Skill is enabled for an app", async () => {
    installedSkillsMock = [
      makeInstalledSkill({ id: "first-id", apps: { grokbuild: true } }),
      makeInstalledSkill({ id: "second-id", apps: { grokbuild: true } }),
    ];
    renderPanel();
  ...
  it("blocks actions but not navigation while checking updates", async () => {
    ...
    expect(screen.getByText("Grok Build:").closest("button")).toBeDisabled();
    expect(screen.getByTitle("skills.uninstall")).toBeDisabled();
  ...
  it("blocks writes immediately when an update check starts", async () => {
    ...
    await userEvent
      .setup()
      .click(screen.getByText("Grok Build:").closest("button")!);
  ...
  it("renders an omp app toggle in omp view when omp is enabled", () => {
    installedSkillsMock = [
      makeInstalledSkill({
        id: "skill-1",
        name: "Omp Skill",
    ...
    render(<UnifiedSkillsPanel onOpenDiscovery={() => {}} currentApp="omp" />);

    // omp 原生支持 skills（exists=active），omp 视图渲染完整开关列
    const ompToggle = screen.getByRole("button", { name: "Oh My Pi" });
    expect(ompToggle).toHaveAttribute("aria-pressed", "true");
    expect(screen.getByText("Omp Skill")).toBeInTheDocument();
  });

  it("renders an inactive omp state in omp view", () => {
    installedSkillsMock = [
      makeInstalledSkill({
        id: "skill-1",
        name: "Grok Skill",
        directory: "grok-skill",
        apps: { grokbuild: true, omp: false },
      }),
    ];

    render(<UnifiedSkillsPanel onOpenDiscovery={() => {}} currentApp="omp" />);

    expect(screen.getByRole("button", { name: "Oh My Pi" })).toHaveAttribute(
      "aria-pressed",
      "false",
    );
    expect(screen.getByRole("button", { name: "Grok Build" })).toHaveAttribute(
      "aria-pressed",
      "true",
    );
  });

  it("never renders a Pi toggle: Pi is not in SKILLS_APP_IDS", () => {
    installedSkillsMock = [
      makeInstalledSkill({
        name: "Grok Skill",
        apps: { grokbuild: true, pi: false },
      }),
    ];

    // 无论当前处于哪个应用上下文，Pi 都不再有开关（SKILLS_APP_IDS 只含 grokbuild/omp）
    render(<UnifiedSkillsPanel onOpenDiscovery={() => {}} currentApp="pi" />);
    expect(
      screen.queryByRole("button", { name: "Pi" }),
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Grok Build" }),
    ).toBeInTheDocument();
