...
describe("SessionManagerPage", () => {
  beforeEach(() => {
    ...
    const sessions: SessionMeta[] = [
      ...
      },
      {
        ...
        sessionId: "grokbuild-session-1",
        title: "Claude Session",
        summary: "Claude summary",
        projectDir: "/mock/grokbuild",
        createdAt: 3,
        lastActiveAt: 30,
        sourcePath: "/mock/grokbuild/session-1.jsonl",
        resumeCommand: "grokbuild resume grokbuild-session-1",
      },
      {
        providerId: "codex",
    ...
    const messages: Record<string, SessionMessage[]> = {
      ...
      "grokbuild:/mock/grokbuild/session-1.jsonl": [
  ...
  it("keeps filtered grouped sessions collapsed until expanding the group", async () => {
    ...
    await switchToGroupedView();
    await switchProviderFilter(/Grok Build/i);
    ...
    expect(
      screen.queryByRole("button", { name: /展开或折叠 grokbuild 目录分组/ }),
    ).not.toBeInTheDocument();

    expandDirectoryGroup("grokbuild", "grokbuild");

    expect(
      screen.getByRole("button", { name: /展开或折叠 grokbuild 目录分组/ }),
    ).toBeInTheDocument();
    expect(
