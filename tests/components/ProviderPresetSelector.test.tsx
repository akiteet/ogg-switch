...
describe("ProviderPresetSelector", () => {
  ...
  it("preset 无 icon 且无 theme.icon 时,回落首字母占位保持文字对齐", () => {
    ...
    // 无图标时仍渲染 ProviderIcon 兜底占位（mock 下 data-icon 为空），文字保持对齐
    const icon = button.querySelector('[data-testid="provider-icon"]');
    expect(icon).not.toBeNull();
  ...
  it("custom 按钮同样渲染占位元素,文字与带图标的预设按钮对齐", () => {
    ...
    // custom 按钮改版后用 Plus 图标（svg[aria-hidden]）作占位元素，文字与
    // 带图标的预设按钮对齐
    const placeholder = customButton.querySelector("[aria-hidden]");
