...
describe("GrokBuildProviderForm", () => {
  ...
  it("submits a complete config.toml payload with Grok defaults", async () => {
    ...
    expect(config.models.default).toBe("grok-4.6");
    expect(config.model["grok-4.6"]).toEqual({
      model: "grok-4.6",
      base_url: "https://relay.example.com/v1",
      // 条目显示名默认取模型 id（不再沿用供应商名）
      name: "grok-4.6",
      api_key: "secret-key",
  ...
  it("uses the Codex-style advanced section without redundant Grok fields", () => {
    ...
    // 上下文窗口在「模型配置」条目编辑器里按行编辑（不再有独立 id 的输入框）
    expect(
      container.querySelector<HTMLInputElement>('input[placeholder="500000"]'),
    ).toHaveValue(500000);
...
`;
    ...
    expect(container.querySelector("#grokbuild-profile")).toBeNull();
    // 模型编辑已迁移到「模型配置」条目编辑器（默认模型单输入框不再属于 Grok 表单）
    const modelIdInput = container.querySelector<HTMLInputElement>(
      'input[placeholder="grok-4.6"]',
    );
    expect(modelIdInput?.value).toBe("existing-profile");
    expect(
...
`;
    ...
    // 模型编辑已迁移到「模型配置」条目编辑器；Codex 的默认模型单输入框不再出现
    expect(container.querySelector("#codexDefaultModel")).toBeNull();
