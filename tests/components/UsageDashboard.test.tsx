...
describe("UsageDashboard", () => {
  ...
  it("filters usage queries to Oh My Pi", async () => {
    renderDashboard();

    // 统计过滤按钮来自 KNOWN_APP_TYPES = [grokbuild, omp]（Pi 已不在过滤列表）
    fireEvent.click(screen.getByRole("button", { name: "usage.appFilter.omp" }));
    ...
    expect(useModelStatsMock).toHaveBeenLastCalledWith(
      expect.anything(),
      { appType: "omp", providerName: undefined },
      expect.anything(),
    );
    expect(usageHeroMock).toHaveBeenLastCalledWith(
      expect.objectContaining({ appType: "omp" }),
    );
  });
