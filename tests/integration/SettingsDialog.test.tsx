...
describe("SettingsPage integration", () => {
  ...
  it("allows browsing and resetting directories", async () => {
    ...
    expect(appInput.value).toBe("/home/mock/.ogg-switch");

    fireEvent.click(browseButtons[0]);
    await waitFor(() =>
      expect(appInput.value).toBe("/home/mock/.ogg-switch/picked"),
    );

    fireEvent.click(resetButtons[0]);
    await waitFor(() => expect(appInput.value).toBe("/home/mock/.ogg-switch"));

    // OGG 的可覆盖目录只剩 应用目录 + Grok 目录（Claude/Codex 等行已移除）
    const grokInput = (await screen.findByPlaceholderText(
      "settings.browsePlaceholderGrok",
    )) as HTMLInputElement;
    fireEvent.change(grokInput, { target: { value: "/custom/grok" } });
    await waitFor(() => expect(grokInput.value).toBe("/custom/grok"));

    fireEvent.click(browseButtons[1]);
    await waitFor(() => expect(grokInput.value).toBe("/custom/grok/picked"));

    fireEvent.click(resetButtons[1]);
    await waitFor(() => expect(grokInput.value).toBe("/home/mock/.grok"));
  });
