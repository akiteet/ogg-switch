...
// 只保留实际接入本地代理计费的应用（Claude/Codex/Gemini 已不在本项目支持范围）
const PRICING_APPS = ["grokbuild"] as const;
...
export function PricingConfigPanel() {
  ...
  const [appConfigs, setAppConfigs] = useState<AppConfigState>({
    grokbuild: { multiplier: "1", source: "request" },
  });
      ...
      try {
        const results = await Promise.all(
          PRICING_APPS.map(async (app) => {
            const multiplier = await proxyApi.getDefaultCostMultiplier(app);
            // 计费一律按请求模型：该选项已从 UI 撤下，历史设置不再读取
            return { app, multiplier, source: "request" as PricingModelSource };
          }),
        );
    ...
    try {
      await Promise.all(
        PRICING_APPS.flatMap((app) => [
          ...
          proxyApi.setPricingModelSource(app, "request"),
        ]),
      );
