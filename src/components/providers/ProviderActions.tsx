...
export function ProviderActions({
  ...
  // OMP 的 OAuth（official）条目无成员语义：凭据在 omp 凭据库、每次 load 都会
  // 重新合成，「移除/添加」对其无效——隐藏主按钮（凭据管理走 OAuth 登录/登出）
  const isMembershipMode = isAdditiveMode && !hideMembershipToggle;
  ...
  return (
    <div className="flex items-center gap-1.5">
      {/* disabled:pointer-events-none prevents the native title from firing,
          ...
          恒显示橙色「移除」；「添加」语义由 + 按钮的预设表单承担）。
          omp 的 OAuth（official）条目 hideMembershipToggle=true：无成员语义，
          也不该出现「启用」（omp 无启用概念），整体不渲染主按钮。 */}
      {!(appId === "omp" && hideMembershipToggle) && (
        <span
