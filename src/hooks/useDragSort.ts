...
import { providersApi, ompApi, type AppId } from "@/lib/api";
...
export function useDragSort(providers: Record<string, Provider>, appId: AppId) {
  ...
  const handleDragEnd = useCallback(
    async (event: DragEndEvent) => {
      ...
      try {
        if (appId === "omp") {
          // omp 真源是 ~/.omp/agent（models.yml 无排序概念），顺序写 OGG meta store
          await ompApi.setOmpProvidersOrder(reordered.map((p) => p.id));
        } else {
