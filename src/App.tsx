...
import type { OmpModelRole } from "@/types/omp";
...
const STORAGE_KEY = "ogg-switch-last-app";
...
// 支持会话管理的 App（单一来源）：会话入口显示与会话视图 fallback 共用，
// 避免正列/反列两份列表再次漂移（曾因漏 omp 导致 OMP 会话被弹回）。
const SESSION_SUPPORTED_APP_IDS: readonly string[] = [
  "claude",
  "codex",
  "grokbuild",
  "omp",
...
/** OMP 角色管理页：包装 OmpRoleManager（读 ~/.omp/agent 配置；组件内部自行调
 *  set/delete omp role 并回传本地状态，这里只负责取数与缓存失效）。 */
function OmpRolesView() {
  const queryClient = useQueryClient();
  const { data, isLoading } = useQuery({
    queryKey: ["omp", "config"],
    queryFn: () => ompApi.readOmpConfig(),
  });
  const [roles, setRoles] = useState<OmpModelRole[]>([]);
  useEffect(() => {
...
function App() {
  ...
  useEffect(() => {
    ...
    if (currentView === "ompRoles" && sharedFeatureApp !== "omp") {
      setCurrentView("providers");
      return;
    }
    if (
      currentView === "sessions" &&
      !SESSION_SUPPORTED_APP_IDS.includes(sharedFeatureApp)
    ) {
      setCurrentView("providers");
    }
  ...
  // omp 技能（AppType::Omp 已接入）与 prompt（AGENTS.md，与 Pi 同构）均支持；
  // MCP 与 Pi 同源不支持；openclaw 无技能命令。
  const hasSkillsSupport = sharedFeatureApp !== "openclaw";
  const hasSessionSupport = SESSION_SUPPORTED_APP_IDS.includes(
    sharedFeatureApp,
  );
  const hasMcpSupport = sharedFeatureApp !== "pi" && sharedFeatureApp !== "omp";
  const hasPromptsSupport = true;
  ...
  const handleConfirmAction = async () => {
    ...
    if (action === "remove") {
      ...
      // omp 特例：不走通用 removeFromLiveConfig（omp 不接入 SQLite/AppType 体系）。
      // 「移除」= 仅撤出 live 配置（meta 库快照保留，列表变「添加」可加回）；
      // 彻底删除走垃圾桶（delete_omp_provider，连 meta 一起清）。
      if (activeApp === "omp") {
        try {
          await ompApi.removeOmpProviderFromLive(provider.id);
          await queryClient.invalidateQueries({
            queryKey: ["providers", "omp"],
          });
          await queryClient.invalidateQueries({ queryKey: ["omp", "config"] });
          await providersApi.updateTrayMenu().catch(() => undefined);
          const wasDefault = provider.id === currentProviderId;
          toast.success(
            wasDefault
              ? t("notifications.ompDefaultRemoved", {
                  defaultValue:
                    "已移除。它此前是默认角色，请在角色管理中重新设置默认",
                })
  ...
  const renderContent = () => {
          ...
          return (
            <div className="px-6 flex flex-col flex-1 min-h-0 overflow-hidden">
              <div className="flex-1 overflow-y-auto overflow-x-hidden pb-12 px-1">
                {/* 不做淡入动画：切换应用时新内容从 opacity:0 淡入 + 旧内容立即
                    卸载会产生 1~3 帧近空白（用户感知为"闪一下"），即时切换无空窗 */}
    ...
    // 不做视图切换动画：新内容从 opacity:0 淡入而旧内容已卸载时，内容区会
    // 出现数帧近空白（"点击跳转闪一下"），即时渲染才不会闪烁。
    return content;
  };
                            ...
                            <>
                              ...
                              {activeApp === "omp" && (
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => setCurrentView("ompRoles")}
                                  className="text-muted-foreground hover:text-foreground hover:bg-black/5 dark:hover:bg-white/5 w-8 px-2"
                                  title={t("omp.roleManager.title", {
                                    defaultValue: "角色管理",
                                  })}
                                >
