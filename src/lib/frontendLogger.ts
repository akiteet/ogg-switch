...
export function installGlobalErrorHandlers(
  ...
  const handleUnhandledRejection = (event: PromiseRejectionEvent) => {
    // invoke 的 Err(String) 会以字符串 reject——原始调用栈此刻已丢失，无法恢复；
    // Error 型 reason 的原生栈由 describeError 原样保留。补记 reason 类型便于区分。
    const hint =
      event.reason instanceof Error
        ? undefined
        : `reason type: ${typeof event.reason}`;
    reportFrontendError("unhandledrejection", event.reason, hint);
  };
