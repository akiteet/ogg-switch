...
import type { OmpProviderCategory, OmpProviderConfig } from "@/types/omp";
...
/**
 * 把本机 ~/.omp/agent 的 provider 映射成通用 Provider，供 ProviderList 渲染。
 * settingsConfig 里保留原始 OMP provider，供编辑表单回填。
 */
const ompProviderToProvider = (provider: OmpProviderConfig): Provider => ({
  id: provider.id,
  name: provider.name,
  settingsConfig: { config: JSON.stringify(provider) },
  websiteUrl: provider.websiteUrl,
  category: ompCategoryToProviderCategory(provider.category),
...
const fetchOmpProviders = async (): Promise<ProvidersQueryData> => {
  const config = await ompApi.readOmpConfig();
  const providers: Record<string, Provider> = {};
  for (const provider of config.providers) {
    providers[provider.id] = ompProviderToProvider(provider);
  }
  const defaultRole = config.roles.find((role) => role.role === "default");
  return {
    providers: sortProviders(providers),
    currentProviderId: defaultRole?.providerId ?? "",
  };
};
...
export const useProvidersQuery = (
  ...
  return useQuery({
    ...
    queryFn: async () => {
      // Oh My Pi 的真源是本机 ~/.omp/agent/models.yml + config.yml，
      // 不走通用的 SQLite 供应商表（那里没有 omp 分区，永远为空）。
      if (appId === "omp") {
        try {
          return await fetchOmpProviders();
        } catch (error) {
          console.error("读取 Oh My Pi 配置失败:", error);
