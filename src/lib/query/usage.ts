...
/**
 * OMP 配额窗口（`~/.omp/agent/agent.db:usage_history` + `omp usage --json`）的
 * 共享 hook。卡片（`OmpQuotaFooter`）与用量看板提示（`UsageHero`）共用同一份缓存。
 *
 * 刷新策略：**手动为主**——卡片的刷新按钮带 `refresh: true` 调后端，由 OMP 用
 * 自己的凭据实时查上游（内置 5 分钟缓存防上游 429）；无自动轮询（用户定案：
 * OMP 窗口数据只在 OMP 使用时变化，轮询没有意义）。queryKey 仍挂在
 * `usageKeys.all` 前缀下，保证会话同步的失效事件能命中。
 */
