...
/// 未管理的 Skill（在应用目录中发现但未被 OGG Switch 管理）
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
pub enum AppType {
    ...
    Hermes,
    Pi,
    /// Oh My Pi（pi 的分支）。真源是 ~/.omp/agent 的 models.yml + config.yml，
    ...
    pub fn load() -> Result<Self, AppError> {
        ...
        if is_v1 {
            return Err(AppError::localized(
                ...
                "检测到旧版 v1 配置格式，当前版本已不再支持运行时自动迁移。\n\n解决方案：手动编辑 ~/.ogg-switch/config.json，将顶层结构调整为：\n   {\"version\": 2, \"claude\": {...}, \"codex\": {...}, \"mcp\": {...}}\n\n",
                "Detected legacy v1 config format; runtime auto-migration is no longer supported.\n\nSolution: manually edit ~/.ogg-switch/config.json to adjust the top-level structure:\n   {\"version\": 2, \"claude\": {...}, \"codex\": {...}, \"mcp\": {...}}\n\n",
            ));
        }
