...
///   且不一定等于用户目录，可能导致 `.ogg-switch/grok-switch.db` 路径变化，从而“看起来像数据丢失”。
///
/// ## 测试隔离
///
...
const APP_CONFIG_DIR_NAME: &str = ".ogg-switch";
const LEGACY_APP_CONFIG_DIR_NAME: &str = ".grok-switch";

/// 将旧品牌的数据目录迁移到新的默认位置。
///
/// 只在新目录尚不存在时执行同卷重命名。迁移失败时继续使用旧目录，避免配置或
/// 数据库因目录被占用而不可见；下次启动会再次尝试迁移。
fn migrate_legacy_app_config_dir(default_dir: &Path, legacy_dir: &Path) -> Option<PathBuf> {
    if default_dir.exists() || !legacy_dir.is_dir() {
        return None;
    }

    match fs::rename(legacy_dir, default_dir) {
        Ok(()) => {
            log::info!(
                "Migrated application data directory from {} to {}",
                legacy_dir.display(),
                default_dir.display()
            );
        ...
        Err(error) => {
            log::warn!(
                "Failed to migrate application data directory from {} to {}; continuing to use the legacy directory: {error}",
                legacy_dir.display(),
                default_dir.display()
            );
...
pub fn get_app_config_dir() -> PathBuf {
    ...
    let default_dir = home_dir.join(APP_CONFIG_DIR_NAME);
    let legacy_dir = home_dir.join(LEGACY_APP_CONFIG_DIR_NAME);

    if let Some(resolved_dir) = migrate_legacy_app_config_dir(&default_dir, &legacy_dir) {
        return resolved_dir;
    }

    // 兼容旧版 Windows：当用户环境存在 `HOME` 且与真实用户目录不同，
    // 旧版本可能在 `HOME/.grok-switch/` 下创建/使用了数据。
    #[cfg(windows)]
    {
        if let Ok(home_env) = std::env::var("HOME") {
            ...
            if !trimmed.is_empty() {
                ...
                if !path_eq_lexical(&legacy_home, &home_dir) {
                    let legacy_dir = legacy_home.join(LEGACY_APP_CONFIG_DIR_NAME);
                    if let Some(resolved_dir) =
                        migrate_legacy_app_config_dir(&default_dir, &legacy_dir)
                    {
                        return resolved_dir;
                    }
                }
...
mod tests {
    ...
    #[test]
    fn migrates_legacy_app_config_directory_to_new_default() {
        let home = tempfile::tempdir().unwrap();
        let legacy_dir = home.path().join(LEGACY_APP_CONFIG_DIR_NAME);
        let default_dir = home.path().join(APP_CONFIG_DIR_NAME);
        std::fs::create_dir_all(legacy_dir.join("backups")).unwrap();
        std::fs::write(legacy_dir.join("grok-switch.db"), b"database").unwrap();
        std::fs::write(legacy_dir.join("backups").join("backup.db"), b"backup").unwrap();

        let resolved = migrate_legacy_app_config_dir(&default_dir, &legacy_dir);

        assert_eq!(resolved, Some(default_dir.clone()));
        assert!(!legacy_dir.exists());
        assert_eq!(
            std::fs::read(default_dir.join("grok-switch.db")).unwrap(),
            b"database"
        );
        assert_eq!(
            std::fs::read(default_dir.join("backups").join("backup.db")).unwrap(),
            b"backup"
        );
    }

    #[test]
    fn leaves_legacy_directory_untouched_when_new_default_exists() {
        let home = tempfile::tempdir().unwrap();
        let legacy_dir = home.path().join(LEGACY_APP_CONFIG_DIR_NAME);
        let default_dir = home.path().join(APP_CONFIG_DIR_NAME);
