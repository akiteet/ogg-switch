...
const SQL_EXPORT_HEADER: &str = "-- OGG Switch SQLite 导出";
...
impl Database {
    ...
    fn validate_sql_export(sql: &str) -> Result<(), AppError> {
        ...
        Err(AppError::localized(
            ...
            "仅支持导入由 OGG Switch 导出的 SQL 备份文件。",
            "Only SQL backups exported by OGG Switch are supported.",
        ))
    }
    ...
    {
        let db_path = get_app_config_dir().join("grok-switch.db");
        if !db_path.exists() {
            return Ok(None);
        }
    ...
    /// Validate that the external SQL created a recognizable OGG Switch schema.
    ///
    ...
    fn validate_imported_schema(conn: &Connection) -> Result<(), AppError> {
        ...
        if !missing.is_empty() {
            ...
            return Err(AppError::localized(
                ...
                format!("导入的 SQL 缺少 OGG Switch 必需表：{names}"),
                format!("The imported SQL is missing required OGG Switch tables: {names}"),
            ));
        }
        Ok(())
    }
...
mod tests {
    ...
    impl TestHomeGuard {
        fn new() -> Self {
            ...
            let config_dir = temp_dir.path().join(".ogg-switch");
            std::fs::create_dir_all(&config_dir).expect("create isolated config directory");
            std::fs::File::create(config_dir.join("grok-switch.db"))
    ...
    fn import_rejects_header_only_sql_and_keeps_existing_database() -> Result<(), AppError> {
        ...
        assert!(
            error.to_string().contains("required OGG Switch tables")
                || error.to_string().contains("OGG Switch 必需表"),
    ...
    fn import_rejects_truncated_open_transaction_and_keeps_live_database() -> Result<(), AppError> {
        ...
        let truncated = exported
            .strip_suffix("COMMIT;\nPRAGMA foreign_keys=ON;\n")
            .expect("OGG Switch export should end with a committed transaction");
