...
impl RequestForwarder {
    ...
    async fn forward(
        ...
        if matches!(app_type, AppType::GrokBuild) {
            // Grok Build:以 CLI 请求的模型条目为准解析上游模型(见
            // resolve_grokbuild_upstream_model 的说明)。只有当请求模型不在
            // config.toml 的条目里时,才回落到默认条目。
            let request_model = mapped_body
                .get("model")
        ...
        let mut request_body = if codex_responses_to_chat {
            ...
            // Grok Build 的模型已在上面(GrokBuild 分支)按 config.toml 条目解析完毕,
            // 这里若再调一次,apply_codex_upstream_model 会因供应商没有 modelCatalog
            // 而把 model 无条件覆写成默认条目 —— 那正是"/model 切换无效"的第二个覆写点。
            if !matches!(app_type, AppType::GrokBuild) {
