...
//! - 只保留 Grok Build 的 /grokbuild/* 路由（见 server.rs::build_router）；
//!   Codex 系 transform / SSE 拼装函数因为 Grok Build 走的是 xAI Responses
//!   原生协议而仍在活链上，不能删。
...
/// Grok Build 模型目录:返回当前 live 配置里的全部模型条目,
/// 让 grok CLI 的 /model 选择器能列出并真正切换(目录拉取自 {base_url}/models)。
///
/// 每个条目必须带 `model` 与 `context_window`:CLI 的解析器对缺这两个字段的条目
/// 会整条丢弃(`Skipping model at index N: missing required field ('model' or
/// 'context_window') or invalid types`),最终报 "model catalog fetch returned no
/// models" —— 只回 `id` 是不够的。`name` 决定选择器里显示的标签;缺省时 CLI 会
/// 回落到内置的 Grok/xAI 身份,所以非 xAI 模型会显示成 "grok"。
pub async fn handle_grokbuild_models() -> Result<Json<Value>, ProxyError> {
    let config_toml =
        std::fs::read_to_string(crate::grok_config::get_grok_config_path()).unwrap_or_default();
    let entries = crate::grok_config::list_grok_model_entries(&config_toml);
    Ok(Json(json!({
        "object": "list",
        "data": entries
            .iter()
...
fn codex_proxy_error_json(
    ...
    let message = if upstream_status == Some(413) {
        // 413 来自上游渠道商的网关（典型是 nginx 的 client_max_body_size），不是 OGG
        // Switch 本地代理的限制（本地 DefaultBodyLimit 已放到 200MB）。上游响应体往往是
        // 一整段 nginx HTML，对用户毫无价值，这里替换成明确指向上游 + 可操作的指引，
        // 避免「以为是 OGG Switch 封装了 nginx / 是本地代理的锅」这种反复出现的误解。
        format!(
            concat!(
                ...
                "provider's server-side limit, not an OGG Switch limit. ",
        ...
        let status_fragment = upstream_status
            ...
            .unwrap_or_default();
        format!(
            "OGG Switch local proxy failed while handling Codex endpoint {endpoint}. Provider: {provider_name}; model: {request_model}{status_fragment}; cause: {cause}"
        )
    };
...
fn chat_sse_to_response_value(body: &str) -> Result<Value, ProxyError> {
    ...
    // strict=false 用于残余尾块：截断的半截 JSON 忽略而非报错，否则一个被掐断的
    // 尾块会把已聚合完整的响应误杀成 422。
...
data: [DONE]\n\n";
    ...
    fn codex_proxy_413_points_to_upstream_not_local_proxy() {
        ...
        let message = body["error"]["message"].as_str().unwrap();
        // 不再误导成「本地代理失败」
        assert!(!message.contains("OGG Switch local proxy failed"));
        // 明确指向上游 + 体积超限 + 可操作指引
