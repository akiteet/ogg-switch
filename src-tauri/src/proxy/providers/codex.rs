...
/// OpenAI Chat Completions, even if the local Codex client is talking to OGG
...
/// Messages protocol (`/v1/messages`). The local Codex client always talks to OGG
/// Switch through the Responses API, so OGG Switch bridges Responses ⇄ Anthropic.
///
...
/// Grok Build 专用:按 CLI 请求的模型条目名解析真正的上游模型。
///
/// config.toml 里每个 [model."<条目名>"] 都带自己的 model 字段;grok CLI 的
/// /model 切换的就是条目名。若按 Codex 的 apply_codex_upstream_model 处理
...
pub fn resolve_grokbuild_upstream_model(
    ...
    if !request_model.is_empty() {
        let map = crate::grok_config::grok_model_entry_upstream_map(config_toml);
        if let Some(upstream) = map.get(request_model) {
