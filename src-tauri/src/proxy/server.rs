...
impl ProxyServer {
    ...
    fn build_router(&self) -> Router {
        // grok-switch 硬隔离:仅服务 Grok Build 自身的端点。
        // Claude/Codex/Gemini 的代理路由已整体拆除,本进程不存在转发其他 Agent 流量的能力。
        Router::new()
            // 健康检查
            .route("/health", get(handlers::health_check))
            .route("/status", get(handlers::get_status))
            // Grok Build 模型目录(grok CLI 的 /model 选择器从这里拉取)
            .route(
                "/grokbuild/v1/models",
                get(handlers::handle_grokbuild_models),
            )
            // Grok Build Responses 主端点
            .route(
                "/grokbuild/v1/responses",
                post(handlers::handle_grokbuild_responses),
            )
            // Grok Build Responses Compact(远程压缩,透传)
            .route(
