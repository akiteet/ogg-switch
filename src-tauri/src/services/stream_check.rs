...
impl StreamCheckService {
    ...
    fn resolve_base_url(app_type: &AppType, provider: &Provider) -> Result<String, AppError> {
        ...
        match app_type {
            ...
            // omp 供应商的 settings_config 约定为 {"config": "<OmpProviderConfig JSON 串>"}，
    ...
    /// omp 供应商 baseUrl 提取：settings_config["config"] 是 OmpProviderConfig 的
    /// JSON 串（由 commands/stream_check.rs 转换而来）。OAuth 类供应商无 baseUrl
    /// （凭据由 omp auth-broker 管理），明确报错而非误报配置缺失。
    fn extract_omp_base_url(provider: &Provider) -> Result<String, AppError> {
        let config_text = provider
            .settings_config
            .get("config")
            .and_then(|v| v.as_str())
            .ok_or_else(|| AppError::Message("omp 供应商缺少 config 载荷".to_string()))?;
        let parsed: serde_json::Value = serde_json::from_str(config_text)
            .map_err(|e| AppError::Message(format!("omp 供应商配置解析失败: {e}")))?;
        parsed
            .get("baseUrl")
            .and_then(|v| v.as_str())
            .map(str::trim)
            ...
            .ok_or_else(|| {
                AppError::Message(
                    "该 omp 供应商为 OAuth 登录（凭据由 omp auth-broker 管理），无可探测的 baseUrl"
                        .to_string(),
                )
            })
    }
