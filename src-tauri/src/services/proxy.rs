...
/// 官方供应商在 Grok Build 接管时的处理策略。
///
/// 严格入口（用户显式开启接管）应当报错——用户需要知道官方态无法被接管；
/// 尽力而为入口（代理重启后重投影）应当静默跳过——这里不该因官方态而失败。
#[derive(Clone, Copy)]
enum GrokOfficialPolicy {
    Reject,
    Skip,
}
...
///
/// 原因：接管模式下 `*_MODEL` 必须由 OGG Switch 写成稳定的 Claude 角色别名，
...
impl ProxyService {
    ...
    pub async fn set_takeover_for_app(&self, app_type: &str, enabled: bool) -> Result<(), String> {
        // grok-switch 硬隔离:仅支持 Grok Build。其他 Agent 的接管请求在此直接拒绝,
        // 保证本进程永远不可能改写 Claude/Codex/Gemini 的配置文件。
        if app_type != "grokbuild" {
            return Err(format!(
                "grok-switch 仅支持 Grok Build,拒绝接管 {app_type} 的配置",
            ));
        }
    ...
    fn apply_grok_takeover_fields(config: &mut Value, proxy_base_url: &str) -> Result<(), String> {
        ...
        // 官方供应商不接管:它本来就该保持官方登录 + 官方端点。
        // 若该配置里残留了代理写入的 [endpoints] // 一并清掉,避免官方态仍走代理地址。
        if crate::grok_config::is_official_live_config(config_toml)
            || !crate::grok_config::extract_model_config(config_toml).is_some()
        {
            let cleaned = crate::grok_config::strip_proxy_endpoints(config_toml)
                .map_err(|e| format!("清理 Grok Build 代理端点失败: {e}"))?;
            config["config"] = json!(cleaned);
            return Ok(());
        }
    ...
    async fn takeover_live_configs(&self) -> Result<(), String> {
        // grok-switch 只服务 Grok Build：这里只接管 Grok 的 live 配置。
        // 其他应用的接管分支已删除（见 set_takeover_for_app 的硬拒绝守卫），
        // 本函数不可能改写它们的配置文件。
        // Grok Build 的接管逻辑集中在 apply_grok_takeover_from_provider（三处入口共用）。
        if self.read_grok_live().is_ok() {
            self.apply_grok_takeover_from_provider(GrokOfficialPolicy::Skip)
                .await?;
        }
    ...
    /// Grok Build 接管/重投影的唯一实现（三处入口共用）。
    ///
    /// 批量接管、严格接管、尽力而为接管此前各有一份几乎相同的 Grok 分支，
    /// 差异只在"官方供应商如何处理"，导致新增逻辑时容易只改其中一两处
    /// （例如只修了严格入口、漏掉代理重启后的重投影）。这里合并为唯一实现
    /// 加一个策略参数。
    ///
    /// 以**供应商模板**为准投影，而不是就地改写当前 live：界面上新增/删除的
    /// 模型条目只存在于模板里，就地改 live 会让新条目永远进不来
    /// （/model 列表缺模型），旧条目的残缺字段（如空 api_key）也会被一直保留。
    async fn apply_grok_takeover_from_provider(
        &self,
        official_policy: GrokOfficialPolicy,
    ) -> Result<(), String> {
        let provider = self.require_current_provider_for_app(&AppType::GrokBuild)?;
        ...
        .map_err(|e| format!("构建 Grok Build 有效配置失败: {e}"))?;
        let settings_toml = effective
            .get("config")
            .and_then(Value::as_str)
            .unwrap_or_default();

        if crate::grok_config::is_official_live_config(settings_toml) {
            return match official_policy {
                GrokOfficialPolicy::Reject => Err(
                ...
                GrokOfficialPolicy::Skip => {
                    log::info!("Grok Build 处于官方登录态（无自定义模型表），跳过代理接管");
                    Ok(())
                }
            };
        }
    ...
    /// 接管指定应用的 Live 配置（严格模式：不可接管则返回错误）。
    ///
    /// grok-switch 只服务 Grok Build（set_takeover_for_app 有硬拒绝守卫），
    /// 因此这里只处理 GrokBuild。
    async fn takeover_live_config_strict(&self, app_type: &AppType) -> Result<(), String> {
        match app_type {
            AppType::GrokBuild => {
                // 严格语义：目标 live 必须存在。
                self.read_grok_live()?;
                self.apply_grok_takeover_from_provider(GrokOfficialPolicy::Reject)
                    .await
            }
            _ => Err(
    ...
    /// 接管指定应用的 Live 配置（尽力而为：不可接管则静默跳过）。
    ///
    /// 与严格模式共用 apply_grok_takeover_from_provider，只在官方供应商的
    /// 处理策略上不同：这里跳过而不是报错。
