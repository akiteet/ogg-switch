...
/// SQL 标量表达式：行的「模型 id」。
///
/// 统计分组与 Dashboard 的模型筛选统一按**请求模型**匹配（模型维度就是用来
/// 区分模型 id 的）。`pricing_model` 只参与计价、不再进入这里——否则在路由接管
/// + request 计价下，按模型 id 筛选会选不中（表现为「筛选形同虚设」）。
...
/// 可读名如 "Grok Build (Session)" 也能选中）；模型按 [`effective_model_sql`]
/// （请求模型 id）匹配。
...
mod tests {
    ...
    fn test_provider_and_model_filters_cover_detail_and_rollup() -> Result<(), AppError> {
...

        {
            ...
            // 计价模型与请求模型不同的行：模型筛选按请求模型命中（pricing_model 只参与计价）。
            insert_usage_log(
                &conn,
                "a-2",
                "claude",
        ...
        assert!(models.contains(&"alias-model"));
        ...
        assert_eq!(hourly_req, 1, "仅 a-1 命中（a-2 请求模型不同）");

        // ⑩ 请求日志列表与下拉同口径：供应商名 + 请求模型 id。
        let logs = db.get_request_logs(
            &LogFilters {
                provider_name: Some("Packy".to_string()),
                model: Some("alias-model".to_string()),
                ..Default::default()
            },
            0,
            10,
