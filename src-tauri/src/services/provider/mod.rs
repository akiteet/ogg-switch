...
impl ProviderService {
    ...
    fn extract_credentials(
        ...
        match app_type {
            ...
            // omp（models.yml 供应商）：settings.config 内嵌 OmpProviderConfig JSON，
            // 凭据在其中；apiKey 可能是 $ENV / !cmd 形态（调用方按需解析）。
            AppType::Omp => {
                let config_text = provider
                    ...
                    .and_then(|v| v.as_str())
                    .unwrap_or("");
                let parsed: Value = serde_json::from_str(config_text).unwrap_or(Value::Null);
                let api_key = parsed
                    .get("apiKey")
                    .and_then(|v| v.as_str())
                    .unwrap_or("")
