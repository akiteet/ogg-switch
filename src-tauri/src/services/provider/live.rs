...
pub(crate) fn write_live_snapshot(app_type: &AppType, provider: &Provider) -> Result<(), AppError> {
    match app_type {
        ...
        AppType::Omp => {
            return Err(AppError::InvalidInput(
                "OMP providers use the OMP provider service (models.yml)".to_string(),
            ));
        }
    }
    Ok(())
...
pub fn sync_current_to_live(state: &AppState) -> Result<(), AppError> {
    ...
    for app_type in AppType::all() {
        // Pi 与 omp 都不在 SQLite 供应商体系内（真源是各自的 agent 目录），
        // 没有 DB 当前供应商可同步；omp 走 commands/omp.rs 专用路径。
...
pub fn read_live_settings(app_type: AppType) -> Result<Value, AppError> {
    match app_type {
        ...
        // omp live 快照：返回 agent 目录下两个 YAML 的原文（只读展示用）。
        // 供应商增删改走 commands/omp.rs 的专用路径，不经过 live 同步体系。
        AppType::Omp => {
            let dir = crate::config::get_home_dir().join(".omp").join("agent");
            let mut result = serde_json::Map::new();
            for name in ["models.yml", "config.yml"] {
                let text = std::fs::read_to_string(dir.join(name)).unwrap_or_default();
                result.insert(name.to_string(), Value::String(text));
            }
...
pub fn import_default_config(state: &AppState, app_type: AppType) -> Result<bool, AppError> {
    ...
    let settings_config = match app_type {
        ...
        // omp 供应商不在 SQLite 体系，导入走 commands/omp.rs（import_default_config 不适用）
        AppType::Omp => {
            return Err(AppError::InvalidInput(
