...
pub enum SkillStorageLocation {
    /// OGG Switch 管理目录 (~/.ogg-switch/skills/)
    #[default]
    CcSwitch,
...
impl SkillService {
    ...
    pub fn get_app_skills_dir(app: &AppType) -> Result<PathBuf> {
        ...
        match app {
            ...
            AppType::Omp => {
                // omp 是 pi 的同源分支，技能目录约定相同：agent 目录下的 skills/
                //（omp 原生 --skills/--no-skills 旗标实锤支持 discovery & loading）。
                return Ok(crate::config::get_home_dir()
                    .join(".omp")
                    .join("agent")
                    .join("skills"));
            }
        ...
        let (backup_path, preserved_pi_path, pi_cleanup_incomplete) =
            match Self::require_valid_directory(&skill.directory) {
                Ok(directory) => {
                    ...
                    // 能验证为 OGG Switch 部署的副本；其余路径保留并返回警告。
    ...
    /// 扫描未管理的 Skills
    ///
    /// 扫描各应用目录，找出未被 OGG Switch 管理的 Skills
    pub fn scan_unmanaged(db: &Arc<Database>) -> Result<Vec<UnmanagedSkill>> {
        ...
        if let Ok(ssot_dir) = Self::get_ssot_dir() {
            scan_sources.push((ssot_dir, "ogg-switch".to_string()));
        }
    ...
    /// 从应用目录导入 Skills
    ///
    /// 将未管理的 Skills 导入到 OGG Switch 统一管理
    pub fn import_from_apps(
        ...
        search_sources.push((ssot_dir.clone(), "ogg-switch".to_string()));
        ...
        for selection in imports {
            ...
            // 其他应用保存用户选择；Pi/omp 的 exists=active 必须直接来自原生目录。
