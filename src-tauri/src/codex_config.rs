...
/// provider is routed through OGG Switch.  A dedicated id is an ownership
...
pub fn apply_codex_official_proxy_route(
    ...
    // Clean only OGG Switch's placeholder from every stale provider table. Real
...
/// Whether a live Codex config is the official route projected by OGG Switch.
...
/// Remove only the official takeover route owned by OGG Switch. This is a
...
"#;
    ...
    fn set_catalog_json_some_preserves_user_owned_catalog() {
        // When OGG Switch generates a catalog (Some arm), it must still respect a
