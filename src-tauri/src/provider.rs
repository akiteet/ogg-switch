...
impl Provider {
    ...
    pub fn resolve_usage_credentials(
        ...
        let (base_url, api_key) = match app_type {
            ...
            // omp（models.yml 供应商）：settings.config 是 OmpProviderConfig 的 JSON 串。
            // apiKey 可能是 $ENV / !cmd 的 secret-bridge 形态——调用方按需解析
            //（stream_check 的 omp 分支走 resolve_secret_form）。
            AppType::Omp => {
                let config_text = settings.get("config").and_then(Value::as_str);
...
pub struct ClaudeDesktopModelRoute {
    /// 真实上游模型名，只保存在 OGG Switch 内部，不写入 Claude Desktop profile。
    pub model: String,
