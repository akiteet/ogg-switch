...
//! OGG Switch only manages explicit provider entries in `models.json`.
...
pub(crate) fn replace_pi_provider(
    ...
    if current != expected {
        return Err(AppError::Conflict(format!(
            "Pi provider '{provider_key}' changed outside OGG Switch"
        )));
    }
...
/// Validate the shape OGG Switch can persist as one
...
fn ensure_models_revision(path: &Path, expected_revision: &str) -> Result<(), AppError> {
    ...
    if actual_revision == expected_revision {
        ...
        Err(AppError::Conflict(format!(
            "Pi models.json changed outside OGG Switch: {}",
            path.display()
        )))
    }
}
