...
fn handle_deeplink_url(
    ...
    if !url_str.starts_with("oggswitch://") {
        return false;
    }
...
/// Windows 托盘图标：按当前显示器 DPI 生成与系统托盘槽位**完全一致**的位图。
///
/// 托盘槽位尺寸跟随 DPI（100% → 16px、125% → 20px、150% → 24px、200% → 32px）。
/// 此前固定提供 32×32 单尺寸图，在 125%/150% 下会被系统再缩放到 20/24px 而发糊
///（用户实测反馈）。这里从 128×128 源图盒式降采样到目标尺寸，系统可 1:1 绘制。
#[cfg(target_os = "windows")]
fn windows_tray_icon(scale_factor: f64) -> Option<Image<'static>> {
    const SOURCE_BYTES: &[u8] = include_bytes!("../icons/128x128.png");

    let source = match Image::from_bytes(SOURCE_BYTES) {
        Ok(icon) => icon,
        Err(err) => {
    ...
    let target = ((16.0 * scale_factor).round() as usize).clamp(16, 64);
    let (sw, sh) = (source.width() as usize, source.height() as usize);
    if sw == target && sh == target {
        return Some(source);
    }
    let rgba = downscale_rgba(source.rgba(), sw, sh, target)?;
    Some(Image::new_owned(rgba, target as u32, target as u32))
}

/// 纯 Rust 盒式降采样（按面积平均 + alpha 预乘），避免最近邻缩放的锯齿，
/// 也避免透明边缘出现暗边。
#[cfg(target_os = "windows")]
fn downscale_rgba(src: &[u8], sw: usize, sh: usize, target: usize) -> Option<Vec<u8>> {
    if sw == 0 || sh == 0 || target == 0 || src.len() < sw * sh * 4 {
        return None;
    }
    let mut out = vec![0u8; target * target * 4];
    for ty in 0..target {
        let y0 = ty * sh / target;
        ...
        for tx in 0..target {
            ...
            let o = (ty * target + tx) * 4;
            if alpha_sum > 0 {
                for c in 0..3 {
                    out[o + c] = (rgb[c] / alpha_sum) as u8;
                }
            }
            out[o + 3] = (alpha_sum / count) as u8;
        }
    }
    Some(out)
}
...
pub fn run() {
    ...
    // 清理旧产品（Grok Switch）残留在注册表里的死自启链，止住每次登录的
    // 「无法找到脚本文件 autostart.vbs」弹窗（详见 auto_launch.rs 注释）。
    #[cfg(target_os = "windows")]
    crate::auto_launch::cleanup_stale_autostart_entries();
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化日志（输出到 <app_config_dir>/logs/ogg-switch.log）
            {
                    ...
                    tauri_plugin_log::Builder::default()
                        ...
                        .targets([
                            ...
                            Target::new(TargetKind::Folder {
                                path: log_dir,
                                file_name: Some("ogg-switch".into()),
                            }),
                        ])
                ...
                log::info!("=== OGG Switch v{} started ===", env!("CARGO_PKG_VERSION"));
            }
            ...
            let db_path = app_config_dir.join("grok-switch.db");
            ...
            //
            // 捕获首次运行快照：所有全新装用户都会看到欢迎弹窗介绍 OGG Switch 的工作方式。
            ...
            // 只有 Grok Build 走首启 live 配置导入。OMP / Antigravity 的供应商
            // 不落 SQLite（真源分别是 ~/.omp/agent 与 agy 的 settings.json）；
            // 其余 agent 已在本 fork 下线，不能再扫 AppType::all()，否则每次启动
            // 都会去读已下线 agent 的 live 配置并把它们写进数据库。
            for app_type in [crate::app_config::AppType::GrokBuild] {
            ...
            if app_state.db.is_prompts_table_empty().unwrap_or(false) {
                ...
                // 仅 Grok Build 仍导入提示词文件；其余 agent 已下线。
                for app in [crate::app_config::AppType::GrokBuild] {
            ...
            #[cfg(any(target_os = "linux", all(debug_assertions, windows)))]
            {
                #[cfg(target_os = "linux")]
                {
                    ...
                    // tauri-plugin-deep-link names the file after the binary:
                    // ~/.local/share/com.oggswitch.desktop/applications/ogg-switch-handler.desktop
                    // Only register if .desktop file doesn't exist to avoid overwriting user customizations
                    let should_register = app
                        .path()
                        .data_dir()
                        .map(|d| !d.join("applications/ogg-switch-handler.desktop").exists())
                        .unwrap_or(true);
                ...
                move |event| {
                    ...
                    for (i, url) in urls.iter().enumerate() {
                        ...
                        if handle_deeplink_url(&app_handle, url_str, true, "on_open_url") {
                            break; // Process only first oggswitch:// URL
                        }
                    }
                }
            });
            ...
            #[cfg(not(target_os = "macos"))]
            {
                #[cfg(target_os = "windows")]
                let windows_icon = {
                    // 托盘槽位 = 16px × 显示器缩放（SM_CXSMICON 随 DPI 变化）
                    let scale = app
                        .primary_monitor()
                        .ok()
                        .flatten()
                        ...
                        .unwrap_or(1.0);
                    windows_tray_icon(scale)
                };
                #[cfg(not(target_os = "windows"))]
                let windows_icon = None;

                if let Some(icon) = windows_icon {
            ...
            tauri::async_runtime::spawn(async move {
                ...
                match state.db.migrate_legacy_antigravity_account_providers() {
                    Ok(migrated) if migrated > 0 => {
                        if let Err(e) = crate::settings::set_current_provider(
                            ...
                            log::warn!("回写 Antigravity Official 当前供应商失败: {e}");
                        }
                    }
                    Ok(_) => {}
                    Err(e) => {
                        log::warn!("迁移 Antigravity 账号供应商到独立账号池失败: {e}");
                    }
                }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::ensure_antigravity_official_provider,
            ...
            commands::set_omp_role,
            commands::delete_omp_role,
            commands::omp_auth_status,
    ...
    app.run(|app_handle, event| {
        ...
        #[cfg(target_os = "macos")]
        {
            match event {
                ...
                // 处理通过自定义 URL 协议触发的打开事件（例如 oggswitch://...）
...
// 只有 Grok Build 有本地代理数据面（server.rs 的路由全是 /grokbuild/*）。
// 历史库的 proxy_config 行可能仍带 claude/codex/gemini 且 enabled=true，
// 不再扫它们，避免启动日志报"无法恢复已下线 agent 的代理"。
const PROXY_STARTUP_APP_TYPES: [&str; 1] = ["grokbuild"];
...
fn show_migration_error_dialog(app: &tauri::AppHandle, error: &str) -> bool {
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Consider rolling back to an older OGG Switch version.\n\n\
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            2) If you see “database version is newer”, please upgrade OGG Switch\n\
