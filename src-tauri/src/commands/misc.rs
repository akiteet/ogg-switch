...
pub async fn check_for_updates(handle: AppHandle) -> Result<bool, String> {
    handle
        ...
        .open_url(
            // 发布页：新版本安装包与更新日志
            "https://github.com/akiteet/ogg-switch/releases",
            None::<String>,
        )
...
// Oh My Pi 官方安装脚本（https://omp.sh/install）
const OMP_INSTALL_UNIX: &str =
    "bash -c 'tmp=$(mktemp) && curl -fsSL https://omp.sh/install -o $tmp && bash $tmp; status=$?; rm -f $tmp; exit $status'";
...
async fn get_single_tool_version_impl(
    ...
    let latest_version = match tool {
        ...
        "omp" => {
            // npm 优先：omp 由 bun 全局安装，包就是 npm 上的 @oh-my-pi/pi-coding-agent，
            // 且 npm registry 在 CN 网络可达性远好于 api.github.com（GitHub 直连超时
            // 会长期显示「最新版本 未知」）；GitHub Releases 仅作兜底（对齐 opencode 模式）。
            if let Some(version) =
                fetch_npm_latest_for_tool(&client, "@oh-my-pi/pi-coding-agent", tool, local).await
            {
                Some(version)
            } else {
                fetch_github_latest_version(&client, "can1357/oh-my-pi").await
            }
        }
        "pi" => {
...
fn build_tool_search_paths(tool: &str) -> Vec<std::path::PathBuf> {
    ...
    if tool == "omp" {
        // Oh My Pi 官方安装脚本经 bun 全局安装，落在 ~/.bun/bin；GUI 进程从
        // 开始菜单/桌面图标启动时常继承不到该目录，需显式补齐。
...
/// 定位 omp CLI 可执行文件；找不到时回落裸命令名 "omp"（交给系统 PATH，行为同旧版）。
/// 供 omp.rs 的子进程调用绕开 GUI 进程 PATH 残缺的问题（omp 装在 ~/.bun/bin 等
/// 非 PATH 目录时，裸 `Command::new("omp")` 会静默失败）。
/// 失败回落时记 warn 日志并列出已尝试的候选路径——否则「未检测到 omp CLI」永远
/// 查不到原因（用户实测反馈）。
pub(crate) fn locate_omp_command() -> std::path::PathBuf {
    match locate_default_tool("omp", None) {
        Ok(path) => {
            log::debug!("omp CLI 定位成功: {}", path.display());
            path
        }
        Err(reason) => {
            let scanned: Vec<String> = build_tool_search_paths("omp")
                .iter()
                .map(|dir| dir.display().to_string())
                .collect();
            log::warn!(
                "omp CLI 定位失败（{reason}），回落裸命令名交给系统 PATH。已扫描目录: {}",
                scanned.join("; ")
            );
...
pub async fn open_provider_terminal(
    ...
    // omp 不在 AppType/SQLite 供应商体系内（供应商只存在于 ~/.omp/agent 与
    // omp CLI 凭据库），「打开终端」就是在目标目录开一个能跑 `omp` 的交互终端。
    if app.trim().eq_ignore_ascii_case("omp") {
        let launch_cwd = resolve_launch_cwd(cwd)?;
        launch_omp_terminal(launch_cwd.as_deref())?;
        return Ok(true);
    }
...
/// OMP「打开终端」：在目标目录开一个能跑 `omp` 的交互终端。
/// Windows 优先 Windows Terminal（`-d` 指定起始目录），回退 `start` 新窗口
///（`current_dir` 继承）；其余平台复用 `launch_terminal_running` 的脚本机制。
fn launch_omp_terminal(cwd: Option<&Path>) -> Result<(), String> {
    // 定位器自带回落（找不到时返回裸 "omp"，交给系统 PATH）
    let omp_cmd = locate_omp_command().display().to_string();
    ...
    #[cfg(not(target_os = "windows"))]
    {
        let script = match cwd {
            Some(dir) => format!("cd \"{dir}\" && {omp_cmd}", dir = dir.display()),
            None => omp_cmd,
        };
        launch_terminal_running(&script, "omp-terminal")
    }
}
...
echo "[ogg-switch] Starting: {label}"
echo ""
{cmd}
echo ""
echo "[ogg-switch] Command exited. Press Enter to close."
read -r _
"#,
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let content = format!(
            "@echo off\r\necho [ogg-switch] Starting: {label}\r\necho.\r\n{cmd}\r\necho.\r\necho [ogg-switch] Command exited. Press any key to close.\r\npause >nul\r\ndel \"%~f0\" >nul 2>&1\r\n",
            label = label,
            cmd = command_line,
        );
        std::fs::write(&bat_file, &content).map_err(|e| format!("写入批处理文件失败: {e}"))?;
